      ...
      <SheetContent className=\"w-full sm:max-w-md flex flex-col p-0\">
        <SheetHeader className=\"border-b border-border px-6 py-5\">
          <SheetTitle className=\"font-heading text-2xl flex items-center justify-between\">
            Your Cart
            <span className=\"label-tech text-muted-foreground\">{totalQuantity} items</span>
          </SheetTitle>
        </SheetHeader>
            ...
            <ul className=\"divide-y divide-border\">
              {items.map((i) => (
                <li key={i.key} className=\"flex gap-3 py-4\" data-testid={`cart-item-${i.key}`}>
                  <img src={i.image} alt={i.product_name} className=\"h-16 w-16 rounded-sm object-cover border border-border\" />
